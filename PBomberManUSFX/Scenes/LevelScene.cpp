...
void LevelScene::spawnEnemyGhost(GameTexture texture, AIType _type, const int _positionX, const int _positionY)
{
    ...
    enemyGhost->setPosition(_positionX, _positionY);
    enemyGhost->setSize(scaledTileSize, scaledTileSize);
    enemyGhost->setAIType(_type);

    addObject(enemyGhost);
...
void LevelScene::generateEnemies()
{
    ...
    for (int i = 0; i < randCount(); i++)
    {
        ...
        // spawn enemigo Fantasma
        int textureRand = randTexture();
        spawnEnemyGhost(GameTexture::EnemyGhost,
             randType() == 0 ? AIType::wandering : AIType::chasing,
             fieldPositionX + cellY * scaledTileSize, fieldPositionY + cellX * scaledTileSize);
    }
    /*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
...
{
     //iterate enemies for collision
    for(auto i = 0; i < enemiesGhost.size(); i++)
    {
        // iterate drawables for collision
        for(const auto& collisionObject : collisionsEnemyGhost)
        //for(auto f = 0; f < collisionsEnemyGhost.size(); f++)
        {
            // check for block collision
            if(isCollisionDetected(enemiesGhost[i]->getRect(), collisionObject.second->getRect()))
            {
                // stop moving on collision detection
                enemiesGhost[i]->setMoving(false);
                enemiesGhost[i]->revertLastMove();
            }
        }
        // check for bomb collision
        if(bomb && isCollisionDetected(enemiesGhost[i]->getRect(), bomb->getRect()))
        {
            // stop moving on collision detection
            enemiesGhost[i]->setMoving(false);
            enemiesGhost[i]->revertLastMove();
        ...
        if(player != nullptr)
        {
            ...
            playerRect.w = static_cast<int>(playerRect.w * 0.2);
            playerRect.h = static_cast<int>(playerRect.h * 0.2);
            if(isCollisionDetected(playerRect, enemiesGhost[i]->getRect()))
            {
                // player killed by enemy
                removeObject(player);
                player = nullptr;
        ...
        {
            // can attack?
            if(!enemiesGhost[i]->isMovingToCell() && enemiesGhost[i]->canAttack())
            {
                // check for attack radius
                if(abs(player->getPositionX() + player->getWidth() / 2 - enemiesGhost[i]->getPositionX() -
                    enemiesGhost[i]->getWidth() / 2) < enemiesGhost[i]->getAttackRadius() &&
                    abs(player->getPositionY() + player->getHeight() / 2 - enemiesGhost[i]->getPositionY() -
                        enemiesGhost[i]->getHeight() / 2) < enemiesGhost[i]->getAttackRadius())
                {
                    // start follow to player
                    followToPlayer(enemiesGhost[i].get());
                }
            }
        }
    }
...
void LevelScene::updateBangsCollision()
{
    ...
    for(const auto& bang : bangs)
    {
        ...
        {
            SDL_Rect enemyRect = (*itEnemiesGhost)->getRect();
            enemyRect.w = static_cast<int>(enemyRect.w * 0.2);
            enemyRect.h = static_cast<int>(enemyRect.h * 0.2);
            if (isCollisionDetected(enemyRect, bang->getRect()))
            {
                removeObject(*itEnemiesGhost);
                itEnemiesGhost = enemiesGhost.erase(itEnemiesGhost);
